  ...
  const fetchInitial = useCallback(async () => {
    ...
    try {
      const [profileData, nodesData, bestNodeData, releasesData, subsData] = await Promise.all([
        hyperinsightApi.getAimProfile(aimName),
        hyperinsightApi.getAimNodes(aimName, {
          userLat: userGeo?.lat,
          userLng: userGeo?.lng,
        }),
        hyperinsightApi.getAimBestNode(aimName, {
          userLat: userGeo?.lat,
          userLng: userGeo?.lng,
        }),
        hyperinsightApi.getAimReleases(aimName),
        hyperinsightApi.getSubscriptions(),
      ]);
      ...
      if (profileData?.id != null) {
        try {
          const deploymentsData = await hyperinsightApi.getAimDeployments(profileData.id);
    ...
    const fetchVersionNodes = async () => {
      try {
        ...
        const [nodesData, bestNodeData] = await Promise.all([
          hyperinsightApi.getAimNodes(aimName, opts),
          hyperinsightApi.getAimBestNode(aimName, opts),
        ]);
        setNodes(Array.isArray(nodesData) ? nodesData : []);
        setBestNode(Array.isArray(bestNodeData) ? bestNodeData[0] ?? null : bestNodeData ?? null);
      } catch {
