...
import { Play, CheckCircle, AlertCircle, Loader2, Copy, Check, ExternalLink, Lock, Info } from 'lucide-react';
...
// §9.2: this addon has `nodes:read` only. The original "Run This AIM" /
// "Add to Mosaic" buttons called useNodeConnect().connect(), which wrote
// into core's own local node registry (window.electronAPI.nodes.add) and
// the aim-nodes cache (window.electronAPI.aimNodes.saveNodeData) — both
// architecturally unreachable from an addon webview. "Subscribe to
// Monitor" and "Copy MCP config" below are untouched: subscribe/unsubscribe
// are pure HyperInsight API calls (already ported to main/index.js, reached
// via hyperinsightApi), and the clipboard copy needs no electronAPI at all.
export const AimActivationZone = ({
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      await hyperinsightApi.subscribe(payload);
      const updated: unknown = await hyperinsightApi.getSubscriptions();
  ...
  const handleUnsubscribe = async () => {
    ...
    try {
      await hyperinsightApi.unsubscribe(activeSubscription.id);
      const updated: unknown = await hyperinsightApi.getSubscriptions();
      const updatedSubs = Array.isArray(updated) ? (updated as SubscriptionDto[]) : [];
      onSubscriptionsChange?.(updatedSubs);
    } catch {
    ...
    <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] p-5 space-y-5">
      ...
      <div className="space-y-3">
        {activationState === 'ready' ? (
          <>
            <div className="flex items-start gap-2 p-3 bg-[var(--surfaceAlt)] rounded-lg border border-[var(--border)]">
              <Info size={14} className="text-[var(--textMuted)] mt-0.5 shrink-0" />
              <div>
                <p className="text-xs text-[var(--text)]">
                  To use this AIM in Mosaic's AI assistant, add node{' '}
                  <span className="font-mono">{nodeName}</span> in Mosaic's Nodes section.
                </p>
                <p className="text-xs text-[var(--textMuted)] mt-0.5">
                  This addon can view node/AIM data, not add nodes to your local list.
                </p>
              </div>
            </div>
