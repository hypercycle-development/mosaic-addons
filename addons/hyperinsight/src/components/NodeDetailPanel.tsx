...
import {
    Server, Activity, Cpu, Zap, HardDrive, Wallet, FileJson, ChevronRight, Globe, ChevronDown, DollarSign, CheckCircle2
} from 'lucide-react';
...
import { hyperinsightApi } from '../api';
...
const AimDetailItem = ({ aim, licenseKey }: { aim: RunningAimDetail, licenseKey: string }) => {
    ...
    const handleFetch = async (e: React.MouseEvent) => {
        ...
        setError(null);
        try {
            const data = await hyperinsightApi.getAimManifest(licenseKey, aim.name);
            setManifest(data);
            setStage('viewing');
        } catch (err: unknown) {
            console.error(err);
...
const AccordionSection = ({
    ...
    id: string,
    title: string,
    // lucide-react's icon props type (`size: string | number`) isn't
    // assignable to a narrower `{ size: number }` ComponentType under this
    // TS/React version pairing — loosen to the widely-compatible ElementType.
...
export const NodeDetailPanel: React.FC<NodeDetailPanelProps> = ({ licenseKey, onClose, sidebarOpen = false, toolScores }) => {
  ...
  // §9.2: this addon has `nodes:read` only — no `nodes:write`. The original
  // "Connect to node" / "Disconnect" footer button called
  // window.electronAPI.nodes.add/delete (core's own local node registry),
  // which is architecturally unreachable from an addon webview (no
  // electronAPI at all, and addonAPI.nodes is deliberately read-only). This
  // is replaced with a read-only "is this node already in your Mosaic node
  // list" check via addonAPI.nodes.list() — real signal, no fake button.
  const [registeredInMosaic, setRegisteredInMosaic] = useState<boolean | null>(null);

  useEffect(() => {
    const fetchAll = async () => {
      setLoading(true);
      const [nodeResult, profileResult] = await Promise.all([
        hyperinsightApi.getNodeDetail(licenseKey).catch((e: unknown) => { console.error(e); return null; }),
        hyperinsightApi.getNodeProfile(licenseKey).catch((e: unknown) => { console.warn('[NodeDetailPanel] getNodeProfile failed, using fallback', e); return null; }),
      ]);
      if (nodeResult) setNode(nodeResult as NodeDetail);
      if (profileResult && !(profileResult as { error?: string }).error) setProfile(profileResult as NodeProfileDto);
      else if ((profileResult as { error?: string })?.error) console.warn('[NodeDetailPanel] getNodeProfile failed, using fallback');
      setLoading(false);
    };
    fetchAll();

    window.addonAPI.nodes
      .list()
      .then((nodes) => setRegisteredInMosaic(nodes.some((n) => n.licenseKey === licenseKey)))
      .catch(() => setRegisteredInMosaic(null));
  }, [licenseKey]);
  ...
  return createPortal(
    <>
      ...
      {/* Footer — read-only Mosaic registration status (§9.2: no nodes:write
          from an addon, see comment on registeredInMosaic above). */}
      <div className="p-4 border-t border-[var(--border)] bg-[var(--surface)] mt-auto sticky bottom-0 z-20">
        {registeredInMosaic === true ? (
          <div className="w-full flex items-center justify-center gap-2 text-[var(--success)] py-3 rounded-lg font-medium text-sm bg-[var(--success)]/10 border border-[var(--success)]/20">
            <CheckCircle2 size={18} />
            Already in your Mosaic node list
          </div>
        ) : registeredInMosaic === false ? (
          <div className="w-full text-center text-xs text-[var(--textMuted)] py-2">
            Not in your Mosaic node list yet — add it from Mosaic's Nodes section to connect.
          </div>
        ) : null}
      </div>
    </div>
