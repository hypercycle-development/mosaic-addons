...
// §9.2: no ConnectButton here — this addon has nodes:read only, and the
// original button wrote into core's local node registry
// (window.electronAPI.nodes.add), unreachable from an addon webview.
// Clicking a row still opens NodeDetailPanel, which shows a read-only
// "already in your Mosaic node list" indicator via addonAPI.nodes.list().
...
const MergedNodeRow = ({ row, hasUserGeo, onSelect }: MergedNodeRowProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Reachability (was "Connect" — see the file-header comment above
          ConnectButton's old location for why there's no action here) */}
      <td className="py-2.5 px-3" onClick={e => e.stopPropagation()}>
        {row.isRoutable ? (
          ...
          <Tooltip content="Node is not publicly accessible">
            <span className="text-xs px-2 py-1 rounded bg-gray-700 text-gray-500">
              Unreachable
            </span>
          </Tooltip>
        )}
...
export const AimIntelligenceZone = ({
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        const data = await hyperinsightApi.getAimStats(aimName, range);
